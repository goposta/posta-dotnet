...
internal sealed class PostaTransport(
    ...
    ILogger logger)
{
    ...
    public PostaTransport(HttpClient httpClient, IPostaCredentialProvider credentialProvider)
        : this(httpClient, credentialProvider, NullLogger.Instance)
    {
    }
    ...
    private void LogErrorResponse(System.Net.HttpStatusCode statusCode, HttpMethod method, string routeTemplate, PostaApiError? error)
    {
        ...
        logger.Log(
            ...
            "Posta returned HTTP {StatusCode} ({StatusName}) for {Method} {RouteTemplate}. Error code: {ErrorCode}; error type: {ErrorType}; message: {ErrorMessage}",
            (int)statusCode,
            statusCode,
            method.Method,
            routeTemplate,
    ...
    private static PostaApiError? TryParseError(string? responseBody)
    {
        ...
        try
        {
            return JsonSerializer.Deserialize<PostaErrorResponse>(responseBody, s_jsonOptions)?.Error;
        }
        catch (JsonException)
        {
            return null;
    ...
    private sealed class PostaErrorResponse
    {
        [JsonPropertyName("error")]
        public PostaApiError? Error { get; init; }
    }
